...
#endif
#if defined(CONFIG_TINNO_SMART_CHARGE)
static ssize_t show_Dynamic_Current(struct device *dev, struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "[show]DynamicCurrent = %d\n", DynamicCurrent);
	return sprintf(buf, "%u\n", DynamicCurrent);
}

static ssize_t store_Dynamic_Current(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	int enable;
	battery_log(BAT_LOG_CRTI, "[EM] Not Support Write Function\n");
	sscanf(buf, "%d", &enable);
    DynamicCurrent = enable;
	battery_log(BAT_LOG_CRTI, "[store]DynamicCurrent = %d\n",DynamicCurrent); 
	return size;
}

static DEVICE_ATTR(Dynamic_Current, 0664, show_Dynamic_Current, store_Dynamic_Current);
#endif
...
void mt_battery_GetBatteryData(void)
{
	...
	printk( "yixuhong maxPgFlag start =%d, g_temp_standar_charger_value=%d v=%d \n", maxPgFlag,g_temp_standar_charger_value,v);
	if((ICharging>0)&(v>0)&(pg>1000))
	{
	    ...
	    battery_xlog_printk(BAT_LOG_CRTI, "maxPgFlag=%d, batteryAveragePg=%d, pg=%d, \n",maxPgFlag, batteryAveragePg,pg);
             ...
             //printk( "[zhangxian]  MAX_CHARGING_PG=%d,ICharging=%d MAX_CHARGING_CURRENT=%d,charger_vol=%d \n", MAX_CHARGING_PG,ICharging,MAX_CHARGING_CURRENT,charger_vol);
...
#endif
#if defined(CONFIG_TINNO_SMART_CHARGE)
        ret_device_file = device_create_file(&(dev->dev), &dev_attr_Dynamic_Current);
#endif
	}
