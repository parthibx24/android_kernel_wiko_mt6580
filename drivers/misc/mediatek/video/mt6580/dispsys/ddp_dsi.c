...
void DSI_PHY_clk_setting(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		MIPITX_OUTREGBIT(struct MIPITX_DSI_BG_CON_REG, DSI_PHY_REG[i]->MIPITX_DSI_BG_CON,
				 RG_DSI_BG_CKEN, 1);
		#ifdef CONFIG_PROJECT_P4200_MMX_IN  //fengyongfei add 
		MIPITX_OUTREGBIT(struct MIPITX_DSI_BG_CON_REG,DSI_PHY_REG[i]->MIPITX_DSI_BG_CON,RG_DSI_V04_SEL,0b111);
		#endif //end
		/* step 3 */
		mdelay(1);
...
#if defined (CONFIG_PROJECT_P4200)||defined (CONFIG_PROJECT_V3750)
unsigned int hx8394f_flag=0;
#endif
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	DSI_T0_INS t0;
	#if defined(CONFIG_PROJECT_V3750)
    DSI_T0_INS t1;
	#endif
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		for (i = 0; i < 3; i++) {
			...
			t0.Data_ID =
			    (t0.Data0 <
			     0xFF) ? DSI_DCS_READ_PACKET_ID : DSI_GERNERIC_READ_LONG_PACKET_ID; // by peter
#else
			t0.Data_ID =
			    (t0.Data0 <
			     0xB0) ? DSI_DCS_READ_PACKET_ID : DSI_GERNERIC_READ_LONG_PACKET_ID;
...
#if defined (CONFIG_PROJECT_P4200)||defined (CONFIG_PROJECT_V3750)
		if(dsi_params->esd_check_for8394f_enable == 1) //dsi_config->
		{ 
			
			   result1 = memcmp((void *)&(dsi_params->lcm_esd_check_table[i].para_list[0]),
				   (void *)&read_data1, recv_data_cnt);
			 //  printk("DSI wrwaknet1a  : %d\n", result1);
	
			   if( hx8394f_flag==1) // flag say this turn error pass set flag to 0 by peterwang
			   {
				   result=0;
				   if(i==2)
				   {
			   ...
			   else if(result1!=0 && hx8394f_flag==0) // first error recovery but set flag
			   {
				   result=1;
				   hx8394f_flag=1;
			   }
			...
			else
			{
				...
				DISPCHECK("DSI read short packet size: %d\n", recv_data_cnt);
 				...
 				if(dsi_params->esd_check_for8394f_enable == 1) //dsi_config->
                 { 
				     // printk("wrwaknet2 in in in!\n");
				      
				     result1 = memcmp((void *)&(dsi_params->lcm_esd_check_table[i].para_list[0]),
										     (void *)&read_data0.byte1, recv_data_cnt);

				    //  printk("DSI wrwaknet2a  : %d\n", result1);
				    
				      if(result1!=0 && hx8394f_flag==1) 
				      {
				      	  result=1;
				      }
				      else if(result1!=0 && hx8394f_flag==0)
				      {
					      result=0;
					      hx8394f_flag=1;
				      }
