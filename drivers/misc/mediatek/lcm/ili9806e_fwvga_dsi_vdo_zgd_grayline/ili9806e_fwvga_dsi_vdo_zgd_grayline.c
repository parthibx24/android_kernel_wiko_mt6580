...
static unsigned int lcm_compare_id(void)
{
	...
	MDELAY(2);
        ...
        DAreg = buffer[0]; ////////////////////////0xA1
        LCM_DBG("ILI9806:  DAreg grayline =%x\n", DAreg);
...
/*
    ...
    */
     if ((LCM_ID == id)&&(lcm_vol < MIN_VOLTAGE)){
        LCM_DBG("[adc_uboot]: find lcm id = %x lcm_vol = %d\n",id,lcm_vol);
        if(DAreg == 0xA1){ // for gray line lcd
...
LCM_DRIVER ili9806e_fwvga_dsi_vdo_zgd_grayline_lcm_drv = 
{
    .name			= "ili9806e_fwvga_dsi_vdo_zgd_grayline",
