...
#if defined(CONFIG_PROJECT_V2600_SINGLE_SW_MULTI_HW)
int ret1 = 0; 
int ret2 = 0;	
char band_buf[4];
...
ret2 = __gpio_get_value(21);
	...
	for (i = 0; i < (sizeof(type_str) / sizeof(char *)); i++) {
		...
		if((ret1 == 0)&&(ret2 == 0))
		{
			//sprintf(buf,	"B1B2B5");
			//sprintf(band_buf,  "1");
			snprintf(img_name, 32, fmt, md_id + 1, type_str[i],"_1");
		}
		else if((ret1 == 0)&&(ret2 == 1))
		{
			//sprintf(buf,	"B2B4B5");
			//sprintf(band_buf,  "2");
			snprintf(img_name, 32, fmt, md_id + 1, type_str[i],"_2");
		}
		else
		{
			//sprintf(buf,	"B1B2B8");
			//sprintf(band_buf,  "3");
			snprintf(img_name, 32, fmt, md_id + 1, type_str[i],"_3");
		}
