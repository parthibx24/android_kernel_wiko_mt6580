...
/// wanglj, DATE20151215, NOTE, Bug ONEWIKO-113 START {
#ifdef CONFIG_WIKO_UNIFY	
	switch(feature_val)
	{
		case modem_2g:
			snprintf(YY_K, 8, "_2g_%s",buf);
			break;
			
		case modem_3g:
			snprintf(YY_K, 8, "_3g_%s",buf);
			break;
			
		case modem_wg:
			snprintf(YY_K, 8, "_wg_%s",buf);
			break;
			
		case modem_tg:
			snprintf(YY_K, 8, "_tg_%s",buf);
			break;			
			
		default:
			break;
	}
#elif defined(CONFIG_PROJECT_V2600_SINGLE_SW_MULTI_HW)
{
		int ret1 = 0; 
		int ret2 = 0;	
		char band_buf[4];

		ret1 = __gpio_get_value(84);
		ret2 = __gpio_get_value(21);
	...
	switch (feature_val) {
	case modem_2g:
		snprintf(YY_K, 8, "_2g_n_%s",band_buf);
		break;

	case modem_3g:
		snprintf(YY_K, 8, "_3g_n_%s",band_buf);
		break;

	case modem_wg:
		snprintf(YY_K, 8, "_wg_n_%s",band_buf);
		break;

	case modem_tg:
		snprintf(YY_K, 8, "_tg_n_%s",band_buf);
		break;
...
#ifdef CONFIG_WIKO_UNIFY
	printk("===LGC=== get_md_post_fix==%d%s_E%d===\n",X, YY_K, Ex);
	printk("===LGC=== get_md_post_fix==%s_%s===",buf,buf_ex);
#endif
}
/***************************************************************************/
