...
//@xucs added sub alsps sensor in 2016/01/19 start

#if defined(CONFIG_CUSTOM_KERNEL_ALSPS_SUB)
#include <cust_alsps_sub.h>
#include <aal_control_sub.h>
#endif
//@xucs added sub alsps sensor in 2016/01/19 end
...
#if defined(CONFIG_CUSTOM_KERNEL_ALSPS_SUB)
/* AAL sub functions********************************************************************** */
int hwmsen_aal_sub_enable(int en)
{
	struct hwmsen_context *cxt = NULL;
	int err = 0;
	...
	if (!hwm_obj) {
		HWM_ERR("AAL hwmdev obj pointer is NULL!\n");
		return -EINVAL;
	} else if ((hwm_obj->dc->cxt[ID_LIGHT_SUB]) == NULL) {
		HWM_ERR("the sensor (%d) is not attached!!\n", ID_LIGHT_SUB);
		return -ENODEV;
	} 

	mutex_lock(&hwm_obj->dc->lock);
	cxt = hwm_obj->dc->cxt[ID_LIGHT_SUB];

	if (en == 1) {
		if ((hwm_obj->active_data_sensor & (1LL << ID_LIGHT_SUB)) == 0) {
			HWM_LOG("enable sensor(%d) by AAL sub operation\n", ID_LIGHT_SUB);
			if (cxt->obj.
			    sensor_operate(cxt->obj.self, SENSOR_ENABLE, &en, sizeof(int), NULL, 0,
					   NULL) != 0) {
				HWM_ERR("enable sensor(%d) err = %d\n", ID_LIGHT, err);
...
int hwmsen_aal_sub_get_data(void)
{
	struct hwmsen_context *cxt = NULL;
	int err = 0;
	int out_size;
	...
	HWM_LOG("hwmsen_aal_sub_get_data\n");
	if (!hwm_obj) {
		HWM_ERR("AAL hwmdev obj pointer is NULL!\n");
		return -EINVAL;
	} else if ((hwm_obj->dc->cxt[ID_LIGHT_SUB]) == NULL) {
	...
	mutex_unlock(&hwm_obj->dc->lock);
	HWM_LOG("hwmsen_aal_sub_get_data3\n");
	return als_data;
}

int hwmsen_aal_sub_get_status(void)
{
	return 0;
}
...
//@xucs added sub alsps sensor in 2016/01/19 start
			...
			if ((idx == ID_LIGHT) || (idx == ID_PRESSURE)
			    ...
			    || (idx == ID_LIGHT_SUB) || (idx == ID_PROXIMITY_SUB)) {
#else
			if ((idx == ID_LIGHT) || (idx == ID_PRESSURE)
			    || (idx == ID_PROXIMITY) || (idx == ID_TEMPRERATURE)) {
#endif
...
//@xucs added sub alsps sensor in 2016/01/19 start
#if defined(CONFIG_CUSTOM_KERNEL_ALSPS_SUB)
			if (ID_PROXIMITY == idx || ID_LIGHT == idx || ID_PROXIMITY_SUB == idx || ID_LIGHT_SUB == idx || ID_PRESSURE == idx) {
#else
...
//@xucs added sub alsps sensor in 2016/01/19 start
#if defined(CONFIG_CUSTOM_KERNEL_ALSPS_SUB)
	if ((event_type & (1LL << ID_PROXIMITY_SUB))
	    && SENSOR_INVALID_VALUE == obj_data.sensors_data[ID_PROXIMITY_SUB].values[0]) {
		event_type &= ~(1LL << ID_PROXIMITY_SUB);
		/* HWM_LOG("remove ps event!!!!!!!!!!!\n"); */
	}
#endif
//@xucs added sub alsps sensor in 2016/01/19 end
...
//@xucs added sub alsps sensor in 2016/01/19 start
	...
	if ((sensor == ID_LIGHT) || (sensor == ID_PRESSURE)
		...
		|| (sensor == ID_LIGHT_SUB) || (sensor == ID_PROXIMITY_SUB)) {
#else
	if ((sensor == ID_LIGHT) || (sensor == ID_PRESSURE)
		|| (sensor == ID_PROXIMITY) || (sensor == ID_TEMPRERATURE)) {
#endif
...
//@xucs added sub alsps sensor in 2016/01/19 start
#if defined(CONFIG_CUSTOM_KERNEL_ALSPS_SUB)
#if (defined(CONFIG_CUSTOM_KERNEL_ALSPS)||defined(CONFIG_CUSTOM_KERNEL_ALSPS_SUB))
			if ((sensor == 4 && aal_use == 1)||(sensor == 30 && aal_use_sub == 1)) {
