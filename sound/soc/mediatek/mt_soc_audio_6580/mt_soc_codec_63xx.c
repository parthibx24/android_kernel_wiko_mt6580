...
#ifdef CONFIG_CUSTOM_KERNEL_ALSPS_SUB
	...
	if (enable) {
		...
		if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL] == false
			...
			if (mIsExtSPKUse) {
				...
				//kangqin changed 0x005c->0x007c for 2 line open .info look [FAQ14293]
			} else {
...
static bool TurnOnADcPowerACC(int ADCType, bool enable)
{
	...
	printk("[xucs]TurnOnADcPowerACC start\n");
	printk("[xucs]TurnOnADcPowerACC,ADCType = %d, enable = %d\n",ADCType,enable);
	...
	printk("[xucs]TurnOnADcPowerACC end\n");
	return true;
}
...
{
    printk("[xucs]Audio_ADC1_Set start\n");
    printk("[xucs]Audio_ADC1_Set mAudio_Analog_Mic1_mode is %d\n",mAudio_Analog_Mic1_mode);
    ...
    printk("[xucs]Audio_ADC1_Set end\n");
	return 0;
}
...
{
	pr_warn("%s()\n", __func__);
    printk("[xucs]Audio_ADC2_Set start\n");
    printk("[xucs]Audio_ADC2_Set mAudio_Analog_Mic1_mode is %d\n",mAudio_Analog_Mic2_mode);
	mutex_lock(&Ana_Power_Mutex);
	if (ucontrol->value.integer.value[0]) {
	...
	mutex_unlock(&Ana_Power_Mutex);
    printk("[xucs]Audio_ADC2_Set end\n");
	return 0;
}
...
static int Audio_MicSource1_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	int index = 0;
    printk("[xucs]Audio_MicSource1_Set start\n");
	
    printk("[xucs]ucontrol->value.enumerated.item[0] is %d\n",ucontrol->value.enumerated.item[0]);
    printk("[xucs]ucontrol->value.integer.value[0] is %ld\n",ucontrol->value.integer.value[0]);
	pr_warn("%s()\n", __func__);
	...
	pr_warn("%s() index = %d done\n", __func__, index);
	    ...
	    if(cur_eint_state == 1)
	    {
	        ...
	        mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] = 2;
	    }
	    } else {
		mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] = ucontrol->value.integer.value[0];	
	    }
...
static int Audio_MicSource2_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_warn("%s() index = %d done\n", __func__, index);
		...
		if(cur_eint_state == 1)
		{
	        ...
	        mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_2] = 0;
		}	
	    } else if (GsensorState == 2) {
	    if(cur_eint_state == 1)
	    {
...
#if defined(CONFIG_CUSTOM_KERNEL_ALSPS_SUB)
void receiverSwitch(int n)
{
		if(n == 2)
		{
		    printk("[xucs]Ext reveiver switch to L channel!");
			AudDrv_GPIO_EXTAMP_Select(false);
			AudDrv_GPIO_EXTAMP2_Select(true);
		} else if (n == 1) {
		    printk("[xucs]Ext reveiver switch to R channel!");
			AudDrv_GPIO_EXTAMP_Select(true);
			AudDrv_GPIO_EXTAMP2_Select(false);	
		}
}

void speakerSwitch(int n)
{
    if(n == 2){
	printk("[xucs]Ext amp switch to L channel!");
	AudDrv_GPIO_EXTAMP_Select(true);
	AudDrv_GPIO_EXTAMP2_Select(false);
	udelay(GAP);
	AudDrv_GPIO_EXTAMP_Select(false);
    ...
    printk("[xucs]Ext amp switch to R channel!");
	AudDrv_GPIO_EXTAMP_Select(false);
	AudDrv_GPIO_EXTAMP2_Select(true);
	udelay(GAP);
	//AudDrv_GPIO_EXTAMP_Select(false);
...
void micSwitch(int n)
{
    TurnOnADcPowerACC(13,false);
	TurnOnADcPowerACC(14,false);
    if(n == 2){
	mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] = 2;
	mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_2] = 2;
    TurnOnADcPowerACC(13,true);
	TurnOnADcPowerACC(14,true);
	printk("[xucs]Ext mic switch to refmic!");
    } else if(n == 1) {
	mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] = 0;	
	mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_2] = 0;	
    TurnOnADcPowerACC(13,true);
	TurnOnADcPowerACC(14,true);
    printk("[xucs]Ext mic switch to main!");
    }
}
#endif
